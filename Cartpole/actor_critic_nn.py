...
class actorCritic(object):
    ...
    def create_variables(self):
        ...
        with tf.name_scope("eligibility_traces"):
            self.actor_traces = [tf.Variable(tf.zeros(train_var.get_shape()), trainable=False) for train_var in
                                 actor_network_variables]
            self.critic_traces = [tf.Variable(tf.zeros(train_var.get_shape()), trainable=False) for train_var in
        ...
        with tf.name_scope("compute_pg_gradients"):
            ...
            self.actor_loss = tf.reduce_mean(
                tf.nn.sparse_softmax_cross_entropy_with_logits(self.logprob, self.action_taken))
            ...
            for i, (grad, var) in enumerate(self.actor_gradients):
                ...
                if grad is not None:
                    ...
                    actor_trace_update = self.actor_traces[i].assign(
                        tf.add(grad, tf.mul(self.gamma_lmbda, self.actor_traces[i].value())))
                    self.actor_gradients[i] = (actor_trace_update * self.delta, var)
            ...
            for i, (grad, var) in enumerate(self.critic_gradients):
                ...
                if grad is not None and self.delta != 0.0:
                    ...
                    critic_trace_update = self.critic_traces[i].assign(
                        tf.add(grad / self.delta, tf.mul(self.gamma_lmbda, self.critic_traces[i].value())))
                    self.critic_gradients[i] = (critic_trace_update * self.delta, var)
                elif grad is not None and self.delta == 0.0:
                    critic_trace_update = self.critic_traces[i].assign(
                        tf.mul(self.gamma_lmbda, self.critic_traces[i].value()))
                    self.critic_gradients[i] = (critic_trace_update * self.delta, var)
